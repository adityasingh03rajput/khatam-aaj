...
public class AdvancedAdminPanel extends JFrame {
    ...
    private void showCreateStudentProfile() {
        ...
        JButton biometricBtn = createStyledButton("Add Biometric", INFO);
        biometricBtn.addActionListener(e -> {
            String studentId = studentIdField.getText();
            String studentName = nameField.getText();
            
            if (studentId.isEmpty() || studentName.isEmpty()) {
                JOptionPane.showMessageDialog(this, 
                    "Please enter Student ID and Name first!", 
                    "Info", JOptionPane.INFORMATION_MESSAGE);
                return;
            }
            
            initiateBiometricRegistration(studentId, studentName);
        });
    ...
    private void initiateBiometricRegistration(String studentId, String studentName) {
        log("Initiating biometric registration for: " + studentName + " (" + studentId + ")");
        
        // Show waiting dialog (non-modal so it doesn't block)
        JDialog waitDialog = new JDialog(this, "Biometric Registration", false);
        waitDialog.setSize(500, 400);
        ...
        JLabel statusLabel = new JLabel("<html><center>" +
            "<h2 style='color: #4F46E5;'>Waiting for Mobile Device</h2>" +
            "<p style='font-size: 14px;'><b>Student:</b> " + studentName + "</p>" +
            "<p style='font-size: 14px;'><b>ID:</b> " + studentId + "</p>" +
            "<br>" +
            "<p style='font-size: 13px;'>Please complete on mobile device:</p>" +
            "<p style='font-size: 13px;'>1. Scan fingerprint</p>" +
            "<p style='font-size: 13px;'>2. Scan face</p>" +
            "<br>" +
            "<p style='font-size: 12px; color: #6B7280;'>Waiting for mobile app response...</p>" +
            "</center></html>");
        ...
        logArea.setForeground(new Color(209, 213, 219));
        logArea.setBorder(new EmptyBorder(10, 10, 10, 10));
        logArea.append("Sending request to mobile device...\n");
        
        JButton closeBtn = createStyledButton("Close", ERROR);
        closeBtn.addActionListener(ev -> waitDialog.dispose());
        ...
        new SwingWorker<String, Void>() {
            protected String doInBackground() {
                ...
                return sendPostRequest("/api/biometric/initiate-registration", request.toString());
            }
            ...
            protected void done() {
                try {
                    String response = get();
                    if (response != null) {
                        logArea.append("[OK] Request sent successfully\n");
                        logArea.append("[INFO] Waiting for fingerprint scan...\n");
                        logArea.append("\n");
                        logArea.append("Instructions for mobile device:\n");
                        logArea.append("1. Open LetsBunk app\n");
                        logArea.append("2. You will see a biometric registration prompt\n");
                        logArea.append("3. Follow the on-screen instructions\n");
                        logArea.append("4. Scan fingerprint when prompted\n");
                        logArea.append("5. Scan face when prompted\n");
                        logArea.append("\n");
                        logArea.append("Note: Make sure mobile device is connected\n");
                        logArea.append("to the same network as this admin panel.\n");
                        log("Biometric registration request sent for: " + studentName);
                    } else {
                        logArea.append("[ERROR] Failed to send request\n");
                        logArea.append("Please check server connection.\n");
                        log("Failed to send biometric registration request");
                    }
                } catch (Exception ex) {
